...
**Data mart:** Star schema — 5 dimension tables + `fact_orders` (grain: one order line item, ~2155 rows).
...
This creates the `datamart` schema with 5 dimension tables and `fact_orders`, then inserts all data from staging.
...
│   ├── 02_data_mart.sql         # star schema (5 dims + fact_orders) with INSERTs
│   └── 03_analyses.sql          # 5 analytical queries
...
**Dimensions:** `dim_product`, `dim_customer`, `dim_employee`, `dim_time`, `dim_shipping`
**Fact table:** `fact_orders` — measures: `unit_price`, `quantity`, `discount`, `total_amount`, `freight_allocation`
